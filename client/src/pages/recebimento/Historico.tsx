      ...
      <div className="space-y-6 pb-12">
        ...
        <Card className="p-4 border-emerald-100 bg-emerald-50/50 shadow-sm flex flex-col md:flex-row gap-4 items-center">
          <div className="flex-1 w-full">
            ...
            <Input 
              placeholder="Link da planilha do Banco de Dados..." 
              value={urlPlanilha} 
              onChange={(e) => setUrlPlanilha(e.target.value)} 
              disabled={isVinculado} className="bg-white border-emerald-200" 
            />
          </div>
            ...
            <Card className="p-4 border-slate-200 bg-white shadow-sm">
              ...
              {showFilters && (
                <div className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-4 animate-in slide-in-from-top-2">
                  ...
                  <Input placeholder="Remetente..." value={filtros.remetente} onChange={(e) => setFiltros({...filtros, remetente: e.target.value})} />
                    ...
                    <PieChart>
                      ...
                      >
                        {kpis.grafRemetente.map((_, i) => (
                          <Cell key={i} fill={CORES_MUNDO[(i + 2) % CORES_MUNDO.length]} />
                        ))}
                      </Pie>
                      <Tooltip />
                      <Legend 
